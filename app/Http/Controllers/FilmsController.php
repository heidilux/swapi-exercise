...
class FilmsController extends Controller
{
    ...
    {
        return $this->respondWith($this->apiService->get('films'));
    }

    public function single(string $film, ?string $relation = null): JsonResponse
    {
        try {
            $response = $this->apiService->get('films/?search=' . $film);
        ...
        $relations['Film'] = $resultObject->results[0]->title;
        foreach ($relationResults as $result) {
            $relationResponse = Http::get($result)->object();
            $relations[Str::title($relation)][$relationResponse->name] = $relationResponse->classification ?? $relationResponse;
        }
